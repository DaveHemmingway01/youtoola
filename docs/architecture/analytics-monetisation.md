...
Deduplication uses in-memory numeric route, interaction-cycle, attempt, and action counters. Keys contain no input or result data and disappear on reset or reload. Provider errors are isolated and never block calculation, result display, validation, navigation, copy, share, export, destination access, or lead submission.
